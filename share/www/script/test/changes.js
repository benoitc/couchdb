...
couchTests.changes = function(debug) {
  ...
  run_on_modified_server([{
    ...
    var req = CouchDB.request("POST", "/test_suite_db/_changes?filter=_docids", options);
    var resp = JSON.parse(req.responseText);
    T(resp.results.length === 0);
    ...
    var docids = JSON.stringify(["something", "anotherthing", "andmore"]),
        req = CouchDB.request("GET", "/test_suite_db/_changes?filter=_docids&doc_ids="+docids, options);
    var resp = JSON.parse(req.responseText);
    T(resp.results.length === 2);
    T(resp.results[0].id === "something");
    T(resp.results[1].id === "anotherthing");
    ...
    if (!is_safari && xhr) {
        ...
        xhr = CouchDB.newXhr();
        xhr.open("POST", "/test_suite_db/_changes?feed=continuous&timeout=500&since=7&filter=_docids", true);
