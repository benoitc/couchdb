...
main(_) ->
    test_util:run(78, fun() -> test() end).

test() ->
    jiffy_test_util:test_good(good()),
    jiffy_test_util:test_good(uescaped(), [uescape]),
...
test_utf8([Case | Rest]) ->
    etap:fun_is(
        fun({invalid_json, invalid_string}) -> true; (Else) -> Else end,
        (catch jiffy:encode(Case)),
        lists:flatten(io_lib:format("Invalid utf-8: ~p", [Case]))
    ),
    Case2 = <<34, Case/binary, 34>>,
    etap:fun_is(
        fun({invalid_json, {_, invalid_string}}) -> true; (Else) -> Else end,
